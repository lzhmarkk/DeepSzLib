...
if __name__ == '__main__':
    ...
    for f in tqdm(patient_files, desc="Loading patient"):
        ...
        y = load_truth_data(os.path.join(patient_dir, f + ".txt"), length=x.shape[0], sample_rate=resample_rate)
        all_x.append(x)
        all_y.append(y)
        user_id += 1
    ...
    for f in tqdm(control_files, desc="Loading control"):
        ...
        y = load_truth_data(None, length=x.shape[0], sample_rate=resample_rate)
        all_x.append(x)
        all_y.append(y)
        user_id += 1
