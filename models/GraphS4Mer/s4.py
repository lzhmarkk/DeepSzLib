...
# Try CUDA extension
try:
    from models.GraphS4Mer.kernels.cauchy import cauchy_mult as cauchy_cuda
    from models.GraphS4Mer.kernels.vandermonde import log_vandermonde_cuda
...
def nplr(measure, N, rank=1, dtype=torch.float, diagonalize_precision=True, B_clip=2.0):
    ...
    if (err := torch.sum(
            (_A - _A[0, 0] * torch.eye(N)) ** 2) / N) > 1e-5:  # if not torch.allclose(_A - _A[0,0]*torch.eye(N), torch.zeros(N, N), atol=1e-5):
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        if has_cuda_extension and z.dtype == torch.cfloat and z.device.type == 'cuda':
...
class S4Model(nn.Module):
    def __init__(
        ...
        for _ in range(n_layers):
            if imple == 'S4':
                self.s4_layers.append(S4Block(d_model, dropout=dropout, transposed=True, lr=min(0.001, lr)))
            elif imple == 'S4D':
                self.s4_layers.append(S4D(d_model, d_state=d_model, dropout=dropout, transposed=True, lr=min(0.001, lr)))
            else:
                raise ValueError()
