...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, cross):
        ...
        x = x.reshape(batch * channels, self.out_seg_num, self.d_model)

        # cross = rearrange(cross, 'b ts_d in_seg_num d_model -> (b ts_d) in_seg_num d_model')
        cross = cross.reshape(batch * channels, -1, self.d_model)
        ...
        dec_output = dec_output.reshape(batch, channels, self.out_seg_num, self.d_model)
